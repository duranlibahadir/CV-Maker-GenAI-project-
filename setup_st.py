...
def set_design():
    ...
    st.markdown("<p style='text-align: center; font-size: 30px;'><b>[Sample Generative AI Chatbot]</b></p>",
...
def download_button():
    ...
    def get_user_config():
        ...
        model_options = {
            ...
            # Recommended as it has a 16K token limit, much higher than the other two. This allows longer 'memory recall'.
            "GPT-3.5 Turbo": "gpt-3.5-turbo",  # Fewer token version of above - not recommended
            "GPT-4": "gpt-4"
            # Latest model from OpenAI. Recommended for complex chatbots, though has a lower token limit, higher token usage cost, and slower returns.
        }

        # Display button choices in the sidebar of the app for the user to pick their desired model from the ones just defined.
        st.sidebar.markdown("<b style='color: darkgreen;'>Choose a GPT model:</b>",
                            unsafe_allow_html=True)  # HTML for beautifying the label, not necessary
        ...
        st.sidebar.markdown("<b style='color: darkgreen;'>Choose a temperature (randomness):</b>",
                            unsafe_allow_html=True)
        temperature = st.sidebar.slider("", min_value=0.1, max_value=1.0, value=0.5, step=0.1,
                                        label_visibility="collapsed")

        # Display an input text box to capture user's OpenAI API key so that the chatbot will be able to generate responses
        st.sidebar.markdown("<b style='color: darkgreen;'>Enter OpenAI API Key to use chatbot:</b>",
                            unsafe_allow_html=True)
        api_key = st.sidebar.text_input("", type="password",
                                        label_visibility="collapsed")  # Hides the entered text for privacy
        ...
        use_index = st.sidebar.checkbox("", value=st.session_state.get('use_index', False),
                                        label_visibility="collapsed")
